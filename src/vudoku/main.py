...
st.markdown(
    "<h1 style='text-align: center;'>Vudoku - Visual Sudoku Solver</h1>", unsafe_allow_html=True
)
...
with col1:
    ...
    webrtc_streamer(key="Input", video_processor_factory=OpenCVVideoProcessor)
...
with col2:
    ...
    if btn_pressed:
        ...
        p_board = slr.gateway(string=p_string)  # returns a 81 bit string
        ibg.build_image(p_board)

        solution = cv.cvtColor(
            src=cv.imread(filename="vudoku/assets/images/out/solution.jpg"), code=cv.COLOR_BGR2RGB
        )
...
with col2:
    ...
    if btn_pressed:
        dilated_img = cv.cvtColor(
            src=cv.imread(filename="vudoku/assets/images/out/dilated.jpg"), code=cv.COLOR_BGR2RGB
        )
...
with col1:
    ...
    if btn_pressed:
        contour_img = cv.cvtColor(
            src=cv.imread(filename="vudoku/assets/images/out/contours.jpg"), code=cv.COLOR_BGR2RGB
        )
...
with col2:
    ...
    if btn_pressed:
        flipped_img = cv.cvtColor(
            src=cv.imread(filename="vudoku/assets/images/out/flipped.jpg"), code=cv.COLOR_BGR2RGB
        )
