...
\label{sec:cptpromd}
\hspace{0.5cm} The input image is captured from a light senstive device and then processed before passing it onto the classifier. This is achieved by \textit{scavenger.py} and \textit{extractor.py} modules. The figure \eqref{fig:7.2} is an example input image which undergoes pre-processing.
...
A caveat with the classification model is that it has been trained only on numbers and so its univeral knowledge set consists only of images with digits. But Sudoku puzzle contains empty grids, it is observed that the clasiifier wrongly verdicts an empty cell as a digit. The current workaroud is to skip the empty cells. The classifier spits out digits from the cells that it recognizes and they are encoded into an 81 bits long string.
...
The validitiy of a number is checked by traversing the current row, column and the box. If validated then the operation is repeated. If not then it traces back to the previous empty cell an replaces the current number with an incremented one.
