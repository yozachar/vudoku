...
\subsubsection{Convolution Layer 2D \texorpdfstring{$\times$}{x}  2} % PDF book marks maynot contain such symbols on heading as they serve as bookmarks
...
\begin{minipage}{0.45\textwidth}
    \centering
    \includegraphics[width=0.5\textwidth]{transformed_puzzle.jpg}
    \caption[Sample Input]{\centering Sampel Input}
    \label{fig:7.4}
\end{minipage}\hfill
\begin{minipage}{0.45\textwidth}
...
\section{Main Script}
\label{sec:mainscrpt}
\hspace{0.5cm} This is a small controls script which provides an overall finish to the project. It includes Streamlit \cite{gh:Streamlit:streamlit}, Webcam and/or Streamlit\_Webrtc for the frontend. They manage async operations with the UI. Rest of it is function calls to the imported local modules. The following figure \eqref{fig:7.6} shows the processing of a live image from left to right.

\begin{figure}[!htbp]
    \centering
    \includegraphics[height=0.5\textwidth]{ui_processing.png}
    \caption[Live Image Processing]{Live Image Processing}
    \label{fig:7.6}
\end{figure}
\vspace*{\fill}
%<<<<<<<<<<<<<<<<<<<<<<< Implementation <<<<<<<<<<<<<<<<<<<<<<<
...
I was exploring the \href{https://github.com/google-research/vision\_transformer}{ViT's Jupyter notebook}, simultaneously trying to grab the Sudoku grid from an image using \href{https://docs.opencv.org/3.0-beta/doc/py\_tutorials/py\_imgproc/py\_houghlines/py\_houghlines.html#hough-tranform-in-opencv}{Hough lines transform}, but since the latter was messed up I ended up \href{https://github.com/google-research/vision\_transformer/issues/107}{enquiring} the possibility of using ViT directly. Little did I know that the very structure of ViT couldn't be used for such a task (as it was stated in a \href{https://github.com/google-research/vision\_transformer/issues/107#issuecomment-842877895}{comment} on my thread), or is it... still an unfinished reasearch.
